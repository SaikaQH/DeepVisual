...
public class Conv1DNN extends ConvolutionalNN {
    public Conv1DNN() {
        this.NN_type += "Conv1D";
        this.getStyleClass().add("Conv1D");
    }

    public Conv1DNN(double layout_pos_x, double layout_pos_y, int btn_id) {
        this();
        this.NN_id = btn_id;
        this.NN_name = "Conv1D" + "_" + NN_id;
        this.setText(this.NN_name);
        initNNBtnPosition(layout_pos_x, layout_pos_y);
        initNNAttribute();
    }

    public Conv1DNN(double layout_pos_x, double layout_pos_y, int btn_id, DeepVisualWindowController _parent) {
    ...
    private final int _dilation_rate = 6;  // int
